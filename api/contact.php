...
if (ENABLE_DB_SAVE) {
    try {
        ...
        $sql = "INSERT INTO contact_requests (
            request_type, company_name, contact_person, department, designation,
            email, phone, city, country, website, key_software, total_software,
            total_users, demo_dates_or_message, ip_address, user_agent
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->bind_param(
            "ssssssssssssssss",
            $requestType, $companyName, $contactPerson, $department, $designation,
            $email, $phone, $city, $country, $website, $keySoftware, $totalSoftware,
            $totalUsers, $demoDates, $ip_address, $user_agent
        );
